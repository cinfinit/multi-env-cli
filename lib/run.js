...
// module.exports = function run({ envFilePath, autoCleanup, commandString }) {
...
//   const child = spawn(commandString, { shell: true, stdio: 'inherit', env: { ...process.env, DOTENV_CONFIG_PATH: tempEnvFile } });
...
//   console.log(`Started instance ${instance.id} with PID ${child.pid}`);

//   child.on('exit', () => {
//     if (autoCleanup) fs.unlinkSync(tempEnvFile);

//     let current = JSON.parse(fs.readFileSync(instancesFile));
//     current = current.filter(i => i.id !== instance.id);
...
// module.exports = async function run({ envFilePath, autoCleanup, commandString }) {
//   const projectRoot = process.cwd();
//   const multiEnvDir = path.join(projectRoot, '.multi-env');
//   if (!fs.existsSync(multiEnvDir)) fs.mkdirSync(multiEnvDir);
...
//       if (autoCleanup && fs.existsSync(tempEnvFile)) fs.unlinkSync(tempEnvFile);
//     });
//   }
...
// node ../multi-env-cli/bin/cli.js run --env-file .env.prod --cmd "node server.js" --auto-cleanup
// node ../multi-env-cli/bin/cli.js run --env-file .env.prod --cmd "node server.js" --auto-cleanup